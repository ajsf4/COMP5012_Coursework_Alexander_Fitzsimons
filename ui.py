...
class Graph():
    ...
    def update_ui(self, data, x_label, y_label):
        self.surface.fill((100,100,100))
        self.surface.set_alpha(168)
        ...
        p2 = np.array((self.y_text.get_width()*2, self.size[1] - self.x_text.get_height()*1.5))
        p3 = np.array((self.size[0] - 5         , self.size[1] - self.x_text.get_height()*1.5))
        ...
        if (len(data[0]) > 1):
            ...
            min_x_tx = self.font.render(f"{int(min(data[0]))}", True, self.draw_colour)
            max_x_tx = self.font.render(f"{int(max(data[0]))}", True, self.draw_colour)
            min_y_tx = pg.transform.rotate(self.font.render(f"{min(data[1]):.2e}", True, self.draw_colour), 90)
            max_y_tx = pg.transform.rotate(self.font.render(f"{max(data[1]):.2e}", True, self.draw_colour), 90)
